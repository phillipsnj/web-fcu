<template>
    <v-app id="app">
        ...
        >
            <v-list dense>
                <v-list-item link @click="change_display_component(item.component)" v-for="item in menuItems"
                             :key="item.title">
                    <v-list-item-action>
        ...
        >
            ...
            <v-toolbar-title>
                <span class="white--text text--darken-1 font-weight-bold display-1 roboto">{{ $store.state.layout.layoutDetails.title }}</span>
                <span class="font-weight-light text-md-center"> {{ $store.state.layout.layoutDetails.subTitle }}</span>
            </v-toolbar-title>
        </v-app-bar>
...
<script>
    ...
    import mergETHNIGE from './components/mergETHNIGE'
